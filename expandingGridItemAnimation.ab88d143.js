...
},{"./gsap-core.js":"OKU8","./CSSPlugin.js":"QIAl"}],"MeEH":[function(require,module,exports) {
"use strict";
  ...
  _createClass(Details, [{
    ...
    value: function initEvents() {
      ...
      this.DOM.close.addEventListener('click', function () {
        return _this.isZoomed ? _this.zoomOut() : _this.close();
      });
      this.DOM.magnifier.addEventListener('click', function () {
        return _this.zoomIn();
      });
    }
  }, {
    key: "fill",
    value: function fill(info) {
      this.DOM.img.src = info.img;
      this.DOM.title.innerHTML = info.title;
      this.DOM.deco.style.backgroundImage = "url(".concat(info.img, ")");
      this.DOM.subtitle.innerHTML = info.subtitle;
      this.DOM.price.innerHTML = info.price;
      this.DOM.description.innerHTML = info.description;
    }
  }, {
    key: "getProductDetailsRect",
    value: function getProductDetailsRect() {
      return {
    ...
    value: function open(data) {
      ...
      var rect = this.getProductDetailsRect();
      this.DOM.bgDown.style.transform = "translateX(".concat(rect.productBgRect.left - rect.detailsBgRect.left, "px) translateY(").concat(rect.productBgRect.top - rect.detailsBgRect.top, "px) scaleX(").concat(rect.productBgRect.width / rect.detailsBgRect.width, ") scaleY(").concat(rect.productBgRect.height / rect.detailsBgRect.height, ")");
      this.DOM.bgDown.style.opacity = 1;
      this.DOM.img.style.transform = "translateX(".concat(rect.productImgRect.left - rect.detailsImgRect.left, "px) translateY(").concat(rect.productImgRect.top - rect.detailsImgRect.top, "px) scaleX(").concat(rect.productImgRect.width / rect.detailsImgRect.width, ") scaleY(").concat(rect.productImgRect.height / rect.detailsImgRect.height, ")");
      this.DOM.img.style.opacity = 1;

      _gsap.default.to(this.DOM.bgDown, {
        duration: 0.25,
        ease: 'sine.out',
        translateX: 0,
        translateY: 0,
      ...
      _gsap.default.fromTo([this.DOM.title, this.DOM.deco, this.DOM.subtitle, this.DOM.price, this.DOM.description, this.DOM.cart, this.DOM.magnifier], {
        translateY: function translateY(index, _, targets) {
          return index !== targets.length - 1 ? 50 : 0;
        },
        scale: function scale(index, _, targets) {
          return index === targets.length - 1 ? 0 : 1;
        }
      }, {
        duration: 0.6,
        ease: 'expo.out',
        delay: function delay(index, _, __) {
          return index * 0.06;
        },
        translateY: function translateY(index, _, targets) {
          return index !== targets.length - 1 ? 0 : 0;
        },
        scale: function scale(index, _, targets) {
          return index === targets.length - 1 ? 1 : 1;
        },
        opacity: 1
      });

      _gsap.default.to(this.DOM.bgUp, {
        duration: 0.1,
        ease: 'none',
        opacity: 1
      });

      _gsap.default.fromTo(this.DOM.close, {
        translateY: '100%'
      }, {
        duration: 0.25,
        ease: 'sine.out',
      ...
      _gsap.default.fromTo(DOM.hamburger, {
        translateY: '0%'
      }, {
        duration: 0.25,
        ease: 'sine.out',
    ...
    value: function close() {
      ...
      _gsap.default.to(DOM.hamburger, {
        duration: 0.25,
        ease: 'sine.out',
        translateY: '0%'
      });

      _gsap.default.to(this.DOM.close, {
        duration: 0.25,
        ease: 'sine.out',
        translateY: '100%',
        opacity: 0
      ...
      _gsap.default.to([this.DOM.title, this.DOM.deco, this.DOM.subtitle, this.DOM.price, this.DOM.description, this.DOM.cart, this.DOM.magnifier], {
        duration: 0.2,
        ease: 'none',
        opacity: 0
      });
    ...
    value: function zoomIn() {
      ...
      _gsap.default.fromTo([this.DOM.title, this.DOM.deco, this.DOM.subtitle, this.DOM.price, this.DOM.description, this.DOM.cart, this.DOM.magnifier], {
        ...
        ease: 'sine.out',
        translateY: function translateY(index, _, targets) {
          return index !== targets.length - 1 ? index === 0 || index === 1 ? -50 : 50 : 0;
        },
        scale: function scale(index, _, targets) {
          return index === targets.length - 1 ? 0 : 1;
        },
      ...
      var imgrect = this.DOM.img.getBoundingClientRect();
      var win = {
        w: window.innerWidth,
      ...
      var targets = imgAnimeOpts.targets,
          gsapOpts = _objectWithoutProperties(imgAnimeOpts, ["targets"]);

      _gsap.default.to(targets, gsapOpts);
    ...
    value: function zoomOut() {
      ...
      _gsap.default.to(this.DOM.img, {
        ...
        rotate: 0,
        onComplete: function onComplete() {
          _this4.DOM.img.style.transformOrigin = '0 0';
...
var items = [];
DOM.gridItems.forEach(function (item) {
  return items.push(new Item(item));
});
DOM.details = new Details();
(0, _imagesloaded.default)(document.body, function () {
  return document.body.classList.remove('loading');
});
},{"imagesloaded":"oZ3m","gsap":"QSxm"}]},{},["MeEH"], null)