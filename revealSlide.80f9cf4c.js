...
},{"./gsap-core.js":"OKU8","./CSSPlugin.js":"QIAl"}],"oJ77":[function(require,module,exports) {
"use strict";
...
var Uncover = /*#__PURE__*/function () {
  ...
  _createClass(Uncover, [{
    ...
    value: function layout() {
      ...
      var inner = '';
      inner += "<div class=\"uncover__img\" style='background-image: ".concat(this.DOM.el.style.backgroundImage, "'></div>\n                  <div class=\"uncover__slices uncover__slices--").concat(this.options.orientation, "\">");

      for (var i = 0; i <= this.options.slicesTotal - 1; ++i) {
        inner += "<div class=\"uncover__slice\" style=\"color:".concat(this.options.slicesColor, "\"></div>");
      }
      ...
      if (!animation) {
        ...
        var settings = {
          slices: {
            ...
            translateX: this.options.orientation === 'vertical' ? ['0%', '0%'] : !this.isCovered ? this.options.slicesOrigin.show === 'right' ? ['0%', '100%'] : ['0%', '-100%'] : this.options.slicesOrigin.hide === 'right' ? ['100%', '0%'] : ['-100%', '0%'],
            translateY: this.options.orientation === 'vertical' ? !this.isCovered ? this.options.slicesOrigin.show === 'bottom' ? ['0%', '100%'] : ['0%', '-100%'] : this.options.slicesOrigin.hide === 'bottom' ? ['100%', '0%'] : ['-100%', '0%'] : ['0%', '0%']
          },
          image: {
        ...
        _gsap.default.killTweensOf(this.DOM.slices);

        _gsap.default.killTweensOf(this.DOM.img);

        var _settings$slices = settings.slices,
            delay = _settings$slices.delay,
            targets = _settings$slices.targets,
            translateX = _settings$slices.translateX,
            translateY = _settings$slices.translateY,
            sliceSettings = _objectWithoutProperties(_settings$slices, ["delay", "targets", "translateX", "translateY"]); // let tl = gsap.timeline(); 


        var promises = [_gsap.default.fromTo(targets, {
          translateX: translateX[0],
          translateY: translateY[0]
        }, _objectSpread(_objectSpread({}, sliceSettings), {}, {
          translateX: translateX[1],
          translateY: translateY[1],
          stagger: 0.08
        }))]; // gsap.to(sliceTargets, {...sliceSettings, stagger: 0.1})
        // for (let index = 0; index < settings.slices.targets.length; index++) {
        //     const currentTarget = settings.slices.targets[index];
        //     tl.to(currentTarget, {...sliceSettings, stagger: 0.1}) //start sequencing
        // }
        //   let promises = [gsap.to(settings.slices.targets,settings.slices).finished];

        if (settings.image.duration) {
          // tl.to(settings.image.targets, imageSettings) //start sequencing
          var _settings$image = settings.image,
              _targets = _settings$image.targets,
              imageSettings = _objectWithoutProperties(_settings$image, ["targets"]);

          promises.push(_gsap.default.fromTo(settings.image.targets, {
            scale: 1.3
          }, _objectSpread(_objectSpread({}, imageSettings), {}, {
            scale: 1
          })));
        }
...
var Slideshow = /*#__PURE__*/function () {
  function Slideshow(el) {
    ...
    this.DOM.slides.forEach(function (slide, pos) {
      return _this2.uncoverItems.push(new Uncover(slide.querySelector('.slide__img'), uncoverOpts[pos]));
    });
    this.init();
  }
  ...
  _createClass(Slideshow, [{
    ...
    value: function navigate(pos) {
      ...
      this.isAnimating = true;
      this.uncoverItems[this.current].hide(true).then(function () {
        _this4.DOM.slides[_this4.current].classList.remove('slide--current');

        _this4.current = pos;
        var newItem = _this4.uncoverItems[_this4.current];
        newItem.hide();

        _this4.DOM.slides[_this4.current].classList.add('slide--current');
...
(0, _imagesloaded.default)(document.querySelectorAll('.slide__img'), {
  background: true
}, function () {
...
},{"imagesloaded":"oZ3m","gsap":"QSxm"}]},{},["oJ77"], null)