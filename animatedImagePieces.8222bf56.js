...
},{"ev-emitter":"RT9e"}],"OKU8":[function(require,module,exports) {
"use strict";
...
},{"./gsap-core.js":"OKU8","./CSSPlugin.js":"QIAl"}],"Ls2u":[function(require,module,exports) {
"use strict";
...
var is3DBuggy = navigator.userAgent.indexOf('Firefox') > 0;
...
var Pieces = /*#__PURE__*/function () {
  function Pieces(el, options) {
    ...
    this.options = {
      ...
      delay: 0,
      // Number || Array(random number from [min,max], e.g. [0,300] would set a random delay per piece from 0 to 300)
      ...
      animationDefaults: {
        duration: 0.6,
        ease: 'expo.out',
        delay: function delay(i, t) {
          return i * parseInt(t.dataset.delay) / 1000;
        },
        translateX: function translateX(i, t) {
          return t.dataset.column < _this.getTotalColumns() / 2 ? getRandomInt(50, 100) + 'px' : getRandomInt(-100, -50) + 'px';
        },
        translateY: function translateY(i, t) {
          return getRandomInt(-1000, -800) + 'px';
        },
        opacity: 0
      }
    };
  ...
  _createClass(Pieces, [{
    ...
    value: function layout() {
      ...
      this.imgsrc = this.DOM.el.style.backgroundImage.replace('url(', '').replace(')', '').replace(/\"/gi, ""); // The background image.

      this.DOM.el.style.backgroundImage = this.options.bgimage !== 'none' ? "url(".concat(this.options.bgimage, ")") : 'none'; // Create the pieces and add them to the DOM (append it to the main element).

      this.pieces = [];

      for (var r = 0; r < this.options.pieces.rows; r++) {
        for (var c = 0; c < this.options.pieces.columns; c++) {
          var piece = this.createPiece(r, c);
          piece.style.backgroundPosition = "".concat(-1 * c * 100, "% ").concat(-1 * 100 * r, "%");
    ...
    key: "createPiece",
    value: function createPiece(row, column) {
      var w = Math.round(this.dimensions.width / this.options.pieces.columns);
      var h = Math.round(this.dimensions.height / this.options.pieces.rows);
      var piece = document.createElement('div');
      piece.style.backgroundImage = "url(".concat(this.imgsrc, ")");
      piece.className = 'piece';
      piece.style.width = "".concat(w, "px");
      piece.style.height = "".concat(h, "px");
      piece.style.backgroundSize = "".concat(w * this.options.pieces.columns + 4, "px auto");
      piece.dataset.row = row;
      piece.dataset.column = column;
      piece.dataset.delay = this.options.delay instanceof Array ? getRandomInt(this.options.delay[0], this.options.delay[1]) : this.options.delay;
      this.DOM.el.appendChild(piece);
      this.DOM.el.style.width = "".concat(w * this.options.pieces.columns, "px");
      this.DOM.el.style.height = "".concat(h * this.options.pieces.rows, "px");
      return piece;
    } // Set the pieces background image.

  }, {
    key: "setImage",
    value: function setImage(imgsrc) {
      this.imgsrc = imgsrc;

      var _iterator = _createForOfIteratorHelper(this.pieces),
          _step;
    ...
    value: function initEvents() {
      ...
      var onMouseMoveFn = function onMouseMoveFn(ev) {
        requestAnimationFrame(function () {
          if (!_this2.tilt) {
            if (is3DBuggy) {
              _this2.DOM.el.style.transform = 'none';
            }
          ...
          var rotX = 2 * _this2.options.tilt.maxRotationX / _this2.win.height * mouseposScroll.y - _this2.options.tilt.maxRotationX;
          var rotY = 2 * _this2.options.tilt.maxRotationY / _this2.win.width * mouseposScroll.x - _this2.options.tilt.maxRotationY;
          var transX = 2 * _this2.options.tilt.maxTranslationX / _this2.win.width * mouseposScroll.x - _this2.options.tilt.maxTranslationX;
          var transY = 2 * _this2.options.tilt.maxTranslationY / _this2.win.height * mouseposScroll.y - _this2.options.tilt.maxTranslationY;
          _this2.DOM.el.style.transform = "perspective(1000px) translate3d(".concat(transX, "px, ").concat(transY, "px,0) rotate3d(1,0,0,").concat(rotX, "deg) rotate3d(0,1,0,").concat(rotY, "deg)");
        });
      }; // Window resize.
      ...
      var onResizeFn = debounce(function () {
        ...
        _this2.DOM.el.style.width = _this2.DOM.el.style.height = '';

        var elBounds = _this2.DOM.el.getBoundingClientRect();
        ...
        for (var i = 0, len = _this2.pieces.length; i < len; i++) {
          var w = Math.round(_this2.dimensions.width / _this2.options.pieces.columns);
          var h = Math.round(_this2.dimensions.height / _this2.options.pieces.rows);
          var piece = _this2.pieces[i];
          piece.style.width = "".concat(w, "px");
          piece.style.height = "".concat(h, "px");
          piece.style.backgroundSize = "".concat(w * _this2.options.pieces.columns + 4, "px auto");
          _this2.DOM.el.style.width = "".concat(w * _this2.options.pieces.columns, "px");
          _this2.DOM.el.style.height = "".concat(h * _this2.options.pieces.rows, "px");
        }
      }, 20);
    ...
    key: "animate",
    value: function animate(animeopts) {
      animeopts = animeopts || this.options.animationDefaults;
      var opts = {};
      Object.assign(opts, animeopts);

      _gsap.default.killTweensOf(this.pieces);

      _gsap.default.to(this.pieces, opts);
    }
  }]);
...
;
(0, _imagesloaded.default)(document.body, {
  background: true
}, function () {
  return document.body.classList.remove('loading');
});
var piecesEl = document.querySelector('.content .pieces');
var piecesObj = new Pieces(piecesEl, {
  pieces: {
    rows: 14,
    columns: 12
  }
});
var menuEl = document.querySelector('.page-nav');
var optionsCtrl = document.querySelector('.content__title');
var closeOptionsCtrl = menuEl.querySelector('a.page-nav__item--close');
...
var showOptions = function showOptions() {
  ...
  piecesObj.animate({
    duration: 3,
    delay: function delay(i, t) {
      var elBounds = piecesEl.getBoundingClientRect();
      ...
      var y2 = tBounds.top + tBounds.height / 2;
      ...
      return (-1 * maxDelay / maxDist * dist + maxDelay) / 1000;
    },
    ease: 'expo.out',
    translateX: function translateX(i, t) {
      return t.dataset.column < piecesObj.getTotalColumns() / 2 ? getRandomInt(-400, 0) : getRandomInt(0, 400);
    },
    translateY: function translateY(i, t) {
      return t.dataset.row < piecesObj.getTotalRows() / 2 ? getRandomInt(-400, 0) : getRandomInt(0, 400);
    },
    opacity: 0.2
  });

  _gsap.default.killTweensOf(optionsCtrl);

  _gsap.default.to(optionsCtrl, {
    duration: 0.7,
    ease: 'expo.out',
    scale: 1.2,
    opacity: 0
  ...
  _gsap.default.killTweensOf(menuEl);

  _gsap.default.fromTo(menuEl, {
    scale: 0
  }, {
    duration: 0.7,
    delay: 0.15,
...
var hideOptions = function hideOptions(ev) {
  ...
  piecesObj.animate({
    ...
    delay: function delay(i, t) {
      ...
      var x1 = elBounds.left + elBounds.width / 2;
      var y1 = elBounds.top + elBounds.height / 2;
      var x2 = t.dataset.centerx;
      var y2 = t.dataset.centery;
      var dist = Math.sqrt(Math.pow(x2 - x1, 2) + Math.pow(y2 - y1, 2));
      var maxDist = Math.sqrt(Math.pow(elBounds.left - x1, 2) + Math.pow(elBounds.top - y1, 2));
      var maxDelay = 800;
      return maxDelay / maxDist * dist / 1000;
    },
    ease: 'expo.out',
    translateX: 0,
    translateY: 0,
  ...
  _gsap.default.fromTo(optionsCtrl, {
    scale: 1.1
  }, {
    duration: 0.7,
    delay: 0.2,
...
},{"imagesloaded":"oZ3m","gsap":"QSxm"}]},{},["Ls2u"], null)